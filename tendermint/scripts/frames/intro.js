...
define(["../model/log_entry"], function (LogEntry) {
    return function (frame) {
        ...
        .after(100, function () {
            frame.snapshot();
            frame.model().subtitle = '<h3>Algorithms like <a href="/raft">Raft</a> were created to solve this problem.</h3>'
                          + frame.model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        ...
        .after(100, function () {
          ...
          frame.model().send(client("X"), node("a"), null, function() {
            node("a")._value = "3";
            node("a")._log.push(new LogEntry(model(), 1, 1, "SET 3"));
            layout.invalidate();
          });
          layout.invalidate();
        })
        ...
        .after(100, function () {
          frame.snapshot();
          frame.model().send(node("a"), node("b"), {type:"AEREQ"}, function () {
            node("b")._log.push(new LogEntry(model(), 1, 1, "SET 3"));
            layout.invalidate();
          });
          frame.model().send(node("a"), node("c"), {type:"AEREQ"}, function () {
            node("c")._log.push(new LogEntry(model(), 1, 1, "SET 3"));
            layout.invalidate();
          });
          layout.invalidate();
        })
